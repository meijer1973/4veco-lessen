...
Produces:
  - 1.2 Vraag – hoofdstuk.md / .html / .pdf
  - 1.2 Vraag – antwoorden.md / .html / .pdf
"""
...
# ---------------------------------------------------------------------------
# Front page content (from blueprint chapter 2: Demand)
# ---------------------------------------------------------------------------
...
PARAGRAPHS = [
    {
        "dir": MODULE / "1.2.1 Individuele vraag",
        "para": "1.2.1 Individuele vraag \u2013 paragraaf.md",
        "answers": "1.2.1 Individuele vraag \u2013 antwoorden.md",
        "asset_prefix": "_assets/",
    },
    {
        "dir": MODULE / "1.2.2 Vraagfactoren",
        "para": "1.2.2 Vraagfactoren \u2013 paragraaf.md",
        "answers": "1.2.2 Vraagfactoren \u2013 antwoorden.md",
        "asset_prefix": "_assets/",
    },
    {
        "dir": MODULE / "1.2.3 Van individuele naar collectieve vraag",
        "para": "1.2.3 Van individuele naar collectieve vraag \u2013 paragraaf.md",
        "answers": "1.2.3 Van individuele naar collectieve vraag \u2013 antwoorden.md",
        "asset_prefix": "_assets/",
    },
    {
        "dir": MODULE / "1.2.4 Gemengde opgaven",
        "para": "1.2.4 Gemengde opgaven \u2013 opgaven.md",
        "answers": "1.2.4 Gemengde opgaven \u2013 antwoorden.md",
        "asset_prefix": "_assets/",
    },
]
...
@page {
  ...
  @bottom-left   { content: "Hoofdstuk 2 — Vraag"; font-family: Arial, sans-serif; font-size: 9pt; color: #555; }
...
def rebalance_table_columns(html):
    ...
    for i, (match, cells) in enumerate(zip(tables, parser.tables)):
        ...
        # Check if any column might wrap unnecessarily
        total_chars = sum(max_lens)
        if total_chars == 0:
            continue

        # Build proportional widths with minimum 8%
        min_width = 8
        raw = [max(max_lens[j], 2) for j in range(ncols)]
        total_raw = sum(raw)
        widths = [max(min_width, round(r / total_raw * 100)) for r in raw]

        # Normalize to 100%
        total_w = sum(widths)
        if total_w != 100:
            # Adjust largest column
            widths[widths.index(max(widths))] += 100 - total_w

        # Build colgroup HTML
        cols = ''.join(f'\n<col style="width: {w}%" />' for w in widths)
        colgroup = f'<colgroup>{cols}\n</colgroup>\n'

        # Replace or insert colgroup in this table
...
def assemble_answers():
    ...
    parts = ["# Antwoorden Hoofdstuk 2 \u2014 Vraag"]
...
def md_to_pdf(md, output_stem):
    ...
    front_end = '</div>\n'  # closing tag of .chapter-front
    front_marker = html.find('class="chapter-front"')
    if front_marker != -1:
        # Find the closing </div> of .chapter-front
        # It's the </div> right before the first page-break div
        page_break_idx = html.find('<div style="break-before: page;">', front_marker)
        if page_break_idx != -1:
            # Find the </div> just before the page break
...
if __name__ == "__main__":
    ...
    chapter_md_path = BASE / "1.2 Vraag \u2013 hoofdstuk.md"
