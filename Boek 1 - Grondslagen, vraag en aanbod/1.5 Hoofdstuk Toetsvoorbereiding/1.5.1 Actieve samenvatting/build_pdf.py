...
@page {
  ...
  @bottom-left   { content: "1.5.1 Actieve samenvatting"; font-family: Arial, sans-serif; font-size: 9pt; color: #555; }
...
def wrap_exercises(html):
    """Wrap each Vraag block in a div for page-break control."""
    html = re.sub(
        r'<p><strong>(Vraag \d+)',
...
if __name__ == "__main__":
    ...
    prefix = "1.5.1 Actieve samenvatting"
    for doc_type in ["samenvatting", "antwoorden"]:
        md_file = os.path.join(base, f"{prefix} \u2013 {doc_type}.md")
        pdf_file = os.path.join(base, f"{prefix} \u2013 {doc_type}.pdf")
        if os.path.exists(md_file):
            print(f"\nBuilding {doc_type}...")
            build_pdf(md_file, pdf_file)
        else:
            print(f"Skipping {doc_type}: {md_file} not found")
