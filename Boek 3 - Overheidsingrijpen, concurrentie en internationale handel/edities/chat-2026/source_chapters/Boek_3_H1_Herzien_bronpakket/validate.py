...
expected_ids=[]
for n in range(1,49):
    expected_ids.append(n)
    if n in (22,40,47):expected_ids.append(str(n)+'A')
check('51 uniquely numbered exercises with three explicit additions',[e['number'] for e in ex]==expected_ids)
check('141 subquestions',sum(len(e['questions']) for e in ex)==141)
...
for target in targets['exercises']:
    ...
    if target['number']==48:expected['context']=expected['context'].replace('pagina 38','pagina 46')
    check(f'target {target["number"]}: exact exercise record apart from page navigation',expected==next(e for e in ex if e['number']==target['number']))
...
check('47 SVG/PNG pairs',len(svgs)==47 and svgs==pngs)
...
for g in geom:
    ...
    if 'c' not in g:
        ...
        if g.get('type')=='ledger':
            rects=[el for el in root.findall('.//s:rect',ns) if el.get('data-euros')]
            check(name+': six labelled amount blocks',len(rects)==6)
            for k,(el,record) in enumerate(zip(rects,g['bars'])):
                check(f'{name}: block {k} value/area match',close(float(el.get('data-euros')),record['euros']) and close(float(el.get('width'))/g['scale'],record['euros'],.0001) and close(float(el.get('height')),43))
                check(f'{name}: block {k} SVG position',close(float(el.get('x')),record['x'],.0001) and close(float(el.get('y')),record['y']))
            check(name+': welfare ledger uses one common scale',close(g['before'],g['after']+g['loss']) and g['before']==360 and g['after']==320)
        if g.get('type')=='scale_comparison':
            lines=[el for el in root.findall('.//s:line',ns) if el.get('data-role')=='demand']
            check(name+': both identical demand functions drawn',len(lines)==2)
            for k,(el,panel) in enumerate(zip(lines,g['panels'])):
                pts=[]
                for suffix in ('1','2'):
                    q=(float(el.get('x'+suffix))-panel['origin'][0])/panel['xscale']
                    p=(panel['origin'][1]-float(el.get('y'+suffix)))/panel['yscale']
                    pts.append((q,p))
                check(f'{name}: panel {k} actual endpoints obey same equation',all(close(p,14-.1*q,.0001) for q,p in pts))
                check(f'{name}: panel {k} horizontal coverage',close(pts[0][0],0) and close(pts[1][0],140))
            check(name+': only vertical scale differs',close(g['panels'][0]['xscale'],g['panels'][1]['xscale']) and close(g['panels'][0]['yscale'],2*g['panels'][1]['yscale']))
        continue
...
cases=[
 ...
 ('controlled market B',(20,.2,2,.1,3),{'q0':60,'p0':8,'q':50,'pc':10,'pp':7,'buyer':2,'seller':1}),
 ('new guided ceramics 22A',(30,.2,6,.2,-4),{'q0':60,'p0':18,'q':70,'pc':16,'pp':20,'cs0':360,'ps0':360,'cs':490,'ps':490,'budget':-280,'net':700,'w':20}),
 ('new mixed subsidy 47A',(14,.1,2,.1,-2),{'q0':60,'p0':8,'q':70,'pc':7,'pp':9,'cs0':180,'ps0':180,'cs':245,'ps':245,'budget':-140,'net':350,'w':10}),
 ('new quota market 40A',(26,.2,8,.1,0),{'q0':60,'p0':14}),
...
for name,got,expected in [
 ('new mixed 47A tax revenue',3*45,135),
 ('new mixed 47A private gains',(F(245)-180)*2,130),
 ('new mixed 47A welfare effect',F(130)-140,-10),
 ('new quota 40A price',26-F('.2')*40,18),
 ('new quota 40A supply reading',8+F('.1')*40,12),
 ('new quota 40A nonbinding quantity',min(80,60),60),
 ('new guided22A gains',2*(490-360),260),
 ('new guided22A expenditure on prior sales',60*4,240),
 ('new guided22A triangle loss',F(1,2)*(70-60)*4,20),
...
lengths={'Boek_3_H1_Overheidsingrijpen':48,'Boek_3_H1_Antwoorden':31,'Boek_3_H1_Docenteninformatie':10,'Boek_3_H1_Beoordeling_en_wijzigingen':2}
for name,n in lengths.items():
    ...
    check(name+': PDF bookmarks',len(doc.get_toc())>= (2 if 'Beoordeling' in name else 5))
    doc.close()
sdoc=fitz.open(ROOT/'output/Boek_3_H1_Overheidsingrijpen.pdf')
for page,token in [(2,'3.1.1'),(10,'3.1.2'),(19,'3.1.3'),(28,'3.1.4'),(35,'3.1.5'),(44,'3.1.6')]:
    check(f'TOC section starts p{page}',token in sdoc[page-1].get_text())
check('mixed target sources face questions', 'Bron 1' in sdoc[45].get_text() and 'Opgave 48' in sdoc[46].get_text())
check('chapter ceiling includes overview','Overzicht' in sdoc[47].get_text() and len(sdoc)==48 and len(sdoc)<=50)
sdoc.close()
# Revision-specific evidence: targets and original exercise identities are not silently replaced.
baseline_targets=json.loads((ROOT/'revision_base/targets-authored.json').read_text())
baseline_answers=json.loads((ROOT/'revision_base/answers.json').read_text())
check('six target records byte-equivalent to baseline semantic JSON',targets==baseline_targets)
for n in (7,16,25,34,43,48):
    check(f'baseline target {n} answers preserved',ans[str(n)]==baseline_answers[str(n)])
check('source typo Qsubub removed','Qsubub' not in student)
check('student type size not reduced',(ROOT/'print.css').read_bytes()==(ROOT/'revision_base/print.css').read_bytes())
check('controlled incidence is core theory before target',(ROOT/'3.1.2 manuscript.md').read_text().index('_assets/3.1.2_fig_2.svg')<(ROOT/'3.1.2 manuscript.md').read_text().index('## Uitgewerkt voorbeeld'))
check('subsidy loss image is core theory before target',(ROOT/'3.1.3 manuscript.md').read_text().index('_assets/3.1.3_fig_3.svg')<(ROOT/'3.1.3 manuscript.md').read_text().index('## Uitgewerkt voorbeeld'))
check('added targeted guidance remains optional','22A' in (ROOT/'3.1.3 manuscript.md').read_text().split('## Begeleide inoefening')[1].split('## Zelfstandige oefening')[0] and '40A' in (ROOT/'3.1.5 manuscript.md').read_text().split('## Begeleide inoefening')[1].split('## Zelfstandige oefening')[0])
from build import page_chunks
parts=[p for f in order for p in page_chunks(ROOT/f)]
pmap=json.loads((ROOT/'QA/page_concordance.json').read_text()) if (ROOT/'QA/page_concordance.json').exists() else json.loads((ROOT/'QA/student_page_concordance.json').read_text())
pmap=pmap.get('old_to_new',pmap)
check('original 40 page topics retained in original order',len(pmap)==40 and list(map(int,pmap.values()))==sorted(map(int,pmap.values())))
for n in (4,12,13,21,24,36,40,45):
    check(f'added teaching page {n} within budget',n not in map(int,pmap.values()) and len(parts)==48)
# Check the actual answer front matter, not only the planned TOC.
adoc=fitz.open(ROOT/'output/Boek_3_H1_Antwoorden.pdf')
check('answer introduction describes revised chapter','48 pagina' in adoc[0].get_text())
for page,token in [(2,'3.1.1'),(7,'3.1.2'),(11,'3.1.3'),(17,'3.1.4'),(22,'3.1.5'),(28,'3.1.6')]:
    check(f'answer section {token} starts page {page}',token in adoc[page-1].get_text())
adoc.close()
...
report={'scope':'local reproducible checks; not repository CI, classroom testing or independent specialist review',
        'student_pages':48,'answer_pages':31,'teacher_pages':10,'review_pages':2,'exercises':51,'subquestions':141,'targets':6,
        'figure_pairs':47,'checks':checks,'passed':sum(c['pass'] for c in checks),'failed':sum(not c['pass'] for c in checks)}
