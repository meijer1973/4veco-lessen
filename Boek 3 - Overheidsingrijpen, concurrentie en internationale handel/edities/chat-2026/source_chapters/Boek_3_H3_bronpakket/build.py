...
def render_md(body):
    body=re.sub(r"(</(?:div|figure|table)>)[ \t]*\n?", r"\1\n\n", body)
...
if __name__=='__main__':
    ...
    (ROOT/'3.3 Internationale handel – hoofdstuk.md').write_text('\n\n'.join((ROOT/f).read_text() for f in json.loads((ROOT/'chapter-order.json').read_text())),encoding='utf-8')
    build(pages,'Boek_3_H3_Internationale_handel','Boek 3 · Hoofdstuk 3.3 · Internationale handel')
    for source,stem,title,kind in [('Antwoorden.md','Boek_3_H3_Antwoorden','Antwoorden · Internationale handel','answer'),('Docenteninformatie.md','Boek_3_H3_Docenteninformatie','Docenteninformatie · Internationale handel','teacher')]:
