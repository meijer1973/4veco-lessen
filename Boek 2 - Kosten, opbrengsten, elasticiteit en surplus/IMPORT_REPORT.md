...
3. Lesson `.gitattributes` (exact reviewed blob only), Book 2 README/import record, repository map, lesson roadmap, archive README/relocations/indexes; platform textbook roadmap/ledger/version index, generated lesson/platform inventories and internal dashboard.
4. Only the named task-specific checker, its tests and `build-scripts/reports/roadmap-archive-preservation.test.js`. The latter retains exact equality for all 85 historical parsed lesson ledger rows while validating the one later owner-requested import row separately; it also supports lesson main before this paired import. Duplicate import rows, invented completion and unrelated added/changed rows remain rejected. No shared renderer, target registry, global classifier or CI workflow changes are part of this task.
...
The first full local pair run found only the ledger's frozen-snapshot mismatch (1 failed, 1,987 passed, 8 skipped tests); that exposed the narrow regression-fixture repair above. The PR verification record gives the final SHAs and results after repair. Required hosted platform CI still checks lesson main; the explicit local run checks the named final pair. Mark the existing PRs ready only after preservation, scope, affected tests, required CI and the independent technical review are complete. No new textbook page review is requested.

During continuation, platform main advanced independently through PR #247. It was merged into this task branch without conflicts; the finite platform diff now starts at `e3e0bf8ce9e3f1fee1582932b52ac6e03b28e7b6`, keeping that upstream authoring-policy change separate from this import. The immutable edition/archive source and original lesson baseline remain unchanged.
