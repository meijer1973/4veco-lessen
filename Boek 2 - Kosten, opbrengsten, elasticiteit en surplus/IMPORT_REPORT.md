...
**Workflow conflict:** the existing `check-paragraph-lane-scope.js --lane textbook` reports 310 Part A paths, zero companion paths, 156 unknown paths and the edition-preservation `.gitattributes` rule as a shared-file change. The unknown set contains edition navigation, delivered PDF names and packaged source/build/provenance files outside its legacy paragraph naming conventions. Its exception mechanism does not classify unknown paths. No scope PASS or paired-paragraph CI claim is made. The narrow remaining publication decision is acceptance of this explicit initial-import file boundary under the supplied handoff; adapting the generic classifier or manufacturing paragraph review records is outside this import. The PR remains draft pending that route disposition.
