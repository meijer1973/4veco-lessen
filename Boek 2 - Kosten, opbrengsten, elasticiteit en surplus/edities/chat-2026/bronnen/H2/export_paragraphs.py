...
def export():
    ...
    chapter=fitz.open(ROOT/'output/Boek_2_H2_Elasticiteit.pdf')
    answers=fitz.open(ROOT/'output/Boek_2_H2_Antwoorden.pdf')
    ansstarts={x[1].split()[0]:x[2]-1 for x in answers.get_toc() if x[0]==1 and x[1].startswith('2.2.')}
    ansmd=(ROOT/'2.2 Elasticiteit – antwoorden.md').read_text()
    summary={}
    for sec,title in TITLES.items():
        ...
        atext=re.search(r'^# '+re.escape(sec)+r' [\s\S]*?(?=^# 2\.2\.|\Z)',ansmd,re.M).group()
