...
| GATE-SHARED-TASK-INGEST-REPAIR-1 | Shared Task Context And Ingestion Repair Human Review | no | Renewed direct human comments returned 2026-06-04 with decision `hold_for_playable_repair` again. `SHARED-TASK-INGEST-PLAYABLE-REPAIR-1` repaired mechanics, but the renewed review found target-task design blockers: textbook graph construction is not the primary active task, prompt blocks appear as source, the completed graph is visible before construction, both transforms are over-fragmented, and visual QA/checkers do not yet prove target-task economy. Gate remains open; no closure, generated lesson output, protected reference mutation, source-data mutation, product-route adoption, target-equivalent proof, diagnostics, mastery/sequencing, PV, Scale Gate 1, or student/product use authorized. Next: `SHARED-TASK-INGEST-PLAYABLE-REPAIR-2`. |
...
| SHARED-TASK-INGEST-PLAYABLE-REPAIR-2 | Target-Task Simplification And Visual QA Repair | no | Active repair after renewed `REVISE`. Must simplify textbook evidence to a primary graph-construction-substitute task plus at most two follow-ups, simplify the actual-exam evidence to source values, calculation, and conclusion, move prompts out of source panes, hide completed graph output before graph-construction success, add graph-workspace visual QA proof, add target-task economy reports/checkers, refresh the human-review packet, and keep `GATE-SHARED-TASK-INGEST-REPAIR-1` open. No generated lesson output, protected reference mutation, source-data mutation, product-route adoption, target-equivalent proof, diagnostics, mastery/sequencing, PV, Scale Gate 1, or student/product use authorized. |
