...
|--------|------|-----------|---------------|
| GRAPH-UX-2 | Graph Game + Checkpoint UI Integration | no | **ACTIVE NEXT OPERATIONAL SPRINT AFTER SKILLMAP-OP-1 / USE THE SHARED TASK SHELL FOR GRAPH/TABLE OPERATIONS.** Upgrade graph/table practice by integrating the GAME-UX-3A shell. Add table-value selection, graph reading, economic axis convention, interpolation, point placement or graph-construction substitute, and less-labelled variants. Produce one working route for `1.1.3` and prove that graph game and checkpoint graph tasks use a coherent UI language. Preserve blocks on target-equivalent completion claims, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV, Scale Gate 1, and student/product use. |
...
shared route visible and scoped. GRAPH-UX-2 is now the next dependency before
graph/math/reasoning integration, `L1.7B-Q2`, `GATE-L1.7B-Q2`, or Scale Gate 1
rely on the shared task shell.
...
proof, and active next sprint is GRAPH-UX-2. No target-equivalent completion
...
- Book 2 Part A should wait until the remaining foundation blockers clear.
  ...
  SPEC-ET-1, EX-LESSON-1, GAME-UX-3A, ENGINE-OP-1, and SKILLMAP-OP-1.
  Still-open blockers are GRAPH-UX-2, MATH-UX-2, REASON-UX-2,
...
- Do not scale companion production until the closed foundation prerequisites
  remain intact and ENGINE-OP-1, SKILLMAP-OP-1,
  GRAPH-UX-2, MATH-UX-2, REASON-UX-2, GAME-ARCH-1,
...
- Treat `GATE-MTU-H4` through `MTU-H4C` as closed platform answer-form
  ...
  has since closed route-visibility proof; `GRAPH-UX-2` is now the active
...
- Use the L2.0 exit-ticket checklist, GATE-L1.7B carried flags,
  ...
  route-visibility proof, and the SYNC-4 shared task-shell specification as
  entry conditions for `GRAPH-UX-2`.
...
- Continue companion controlled-scope work only where it supports the L1.7A
  ...
  L1.7C-MATH/L1.7B-MAP/L1.7B-P23/SPEC-ET-1/EX-LESSON-1/GAME-UX-3A/ENGINE-OP-1/SKILLMAP-OP-1 foundation, and the open
  GRAPH-UX-2/MATH-UX-2/REASON-UX-2/GAME-ARCH-1/GATE-ENGINE-1/L1.7B-Q2/
...
- L1.7B-C through GATE-L1.7B closed the checkpoint product boundary only.
  ...
  SKILLMAP-OP-1 is closed as visible-route proof. GRAPH-UX-2 through
  GATE-ENGINE-1 must prove the shared task shell and practice engines
