...
|--------|------|-----------|---------------|
| L1.5Q | Course Blueprint v5 + Four Test-Week Book Plan | no | **ACTIVE CURRICULUM-SOURCE SPRINT / PHASE A IMPLEMENTED.** After the L1.5P print cut, update the full course blueprint to the 12/12/14/16 paragraph distribution. Phase A resolved the v4->v5 source-of-truth migration seam: active target-exercise registry now points to v5 with 54 count-bearing records, v5 owned blueprint metadata exists, v5 count/placeholder validator passes, and owned-source/content/source-document builders now derive active blueprint paths from registry metadata instead of brittle v4 constants. Phase B remains: finish the pedagogical v5 blueprint/migration review without treating placeholders as reviewed final target exercises. |
