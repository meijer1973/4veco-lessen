...
|--------|------|-----------|---------------|
| L1.7B-R | Boundary-Safe Exit Ticket MVP Resume | no | **ACTIVE FOUNDATION-HARDENING SPRINT; REQUIRED BEFORE SCALE GATE 1 UNLESS EXPLICITLY WAIVED BY HUMAN DECISION.** L2.0 closed PASS WITH FLAGS, so the next controlled step is the boundary-safe exit-ticket MVP resume. First resolve source-control/product-boundary issues: import or rewrite the prototype as source-controlled platform code, remove mastery/pass/score/evidence/adaptive-focus language, and consume shared skill-map compact checkpoint mode from `GAME-UX-1`. Build one non-summative checkpoint MVP on one already-built paragraph. It may give feedback and self-check guidance, but must not make grading, mastery, diagnostic, adaptive-routing, sequencing, AI, summative, PV-projection, or PV-machine-promotion claims. Landing-page `Check` activation belongs here unless split later: enable `Check` only for paragraphs with reviewed exit-ticket output and keep it hidden elsewhere. Requires student-experience review, teacher-learning-quality review, screenshot/interaction QA, and generator-owned landing-page integration. |
...
| Scale Gate 1 | Foundation Hardening Scale Gate | no | **FUTURE JOINT DECISION.** L2.0 Book 1 house-style cleanup is now closed PASS WITH FLAGS, but broad companion scaling remains blocked until the following are closed or explicitly waived by human decision: L1.7B-R boundary-safe exit-ticket MVP; GATE-L1.7B exit-ticket product-boundary review; L1.7C/L2.0 game-row flags carried into a scale decision; L1.7D landing-page IA plus L2.0 consolidation route accepted as baseline; minimal curriculum-versioning readiness; and reference-side status on GATE-EX5 / operation-answer-skill contract. Scale Gate 1 may authorize controlled production only, not adaptive diagnostics, mastery/sequencing, student-facing AI, summative use, PV projection, or PV machine promotion. |
...
Completed: no. Status: active foundation-hardening sprint before Scale Gate 1
...
- Continue companion pilot work only where it supports the L1.7A decision and
  the L1.7B-C/L1.7C-0/L1.7C/L1.7D/L2.0/L1.7B-R foundation-hardening sequence,
...
- Use the L1.7A readiness decision, L1.7B-C contract-and-stop outcome,
  ...
  IA closure plus L2.0 house-style closure as the baseline. Complete
  L1.7B-R/GATE-L1.7B and Scale Gate 1 if companion scaling is still desired.
