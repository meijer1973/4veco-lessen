...
|--------|------|-----------|---------------|
| L1.6 | Second Pipeline Regression Paragraph | no | **NEXT ACTIVE LESSON PIPELINE SPRINT.** With L1.5P closed and L1.5Q closed, build the next fresh paragraph against the v5 curriculum-source baseline. Confirm Round 2 layout + graphical game + adaptive-input seam survive a fresh end-to-end build, using the L-PV contract/proof machinery for at least one procedure/visual sequence where PV data exists. Any A61 route needs full student-facing build and teacher-learning-quality review; Proof 002 alone is not enough. |
