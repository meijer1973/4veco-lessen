...
| L1.5B | Layout Round 2 - Generator Items | no | **NEXT ACTIVE / PLAN READY.** Unblocked after L-PV5 for layout/generator work, under the PV-G4 `PASS WITH CONDITIONS` boundary. Plan: `L1.5B-sprint-plan.md`. Scope is generator-owned landing/index polish: per-section card accents, split pitfall presentation where structured data supports it, and regression checks for no procedure drift, no internal-code leakage, no stale three-step wording, and no PV machine/student-facing projection. |
