...
| L1.5B | Layout Round 2 - Generator Items | no | **NEXT ACTIVE / PLAN REQUIRED.** Unblocked after L-PV5 for layout/generator work, under the PV-G4 `PASS WITH CONDITIONS` boundary. Acts on what L1.4 surfaced + the deferred round-2 item #3 (split valkuilen pitfalls) + the deferred item #1 (per-section card accents, reclassified as generator-touching by L1.5A planner). Plan must include `validate-procedure-contracts.js`, no internal-code leakage, no stale three-step wording, and no PV machine/student-facing projection. |
