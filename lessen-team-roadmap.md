...
|--------|------|-----------|---------------|
| MATH-UX-2 | Math Game + Checkpoint UI Integration | no | **ACTIVE NEXT OPERATIONAL SPRINT AFTER GRAPH-UX-2 / USE THE SHARED TASK SHELL FOR CALCULATION OPERATIONS.** Upgrade the scoped math route by integrating numeric input, calculation/work capture, final answer, percentage/index notation, units where relevant, and feedback on common calculation errors. Produce one working route for `1.1.2` showing the relation between skill-map route, math practice, and target-equivalent exit-ticket proof. Preserve blocks on target-equivalent completion claims until `L1.7B-Q2`/`GATE-L1.7B-Q2`, and preserve blocks on diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV, Scale Gate 1, and student/product use. |
...
shared route visible and scoped, and GRAPH-UX-2 has since integrated the shell
into live graph/table output. MATH-UX-2 is now the next dependency before
calculation practice, `L1.7B-Q2`, `GATE-L1.7B-Q2`, or Scale Gate 1 rely on the
shared task shell for calculation operations.
...
active next sprint is MATH-UX-2. No target-equivalent completion
...
- Book 2 Part A should wait until the remaining foundation blockers clear.
  ...
  SPEC-ET-1, EX-LESSON-1, GAME-UX-3A, ENGINE-OP-1, SKILLMAP-OP-1, and
  GRAPH-UX-2.
  Still-open blockers are MATH-UX-2, REASON-UX-2,
  GAME-ARCH-1, GATE-ENGINE-1, L1.7B-Q2,
...
- Do not scale companion production until the closed foundation prerequisites
  remain intact and ENGINE-OP-1, SKILLMAP-OP-1, GRAPH-UX-2,
  MATH-UX-2, REASON-UX-2, GAME-ARCH-1,
...
- Treat `GATE-MTU-H4` through `MTU-H4C` as closed platform answer-form
  ...
  has since closed route-visibility proof, and `GRAPH-UX-2` has since closed
  graph/table task-shell integration; `MATH-UX-2` is now the active
...
- Use the L2.0 exit-ticket checklist, GATE-L1.7B carried flags,
  ...
  route-visibility proof, GRAPH-UX-2 graph/table task-shell integration, and
  the SYNC-4 shared task-shell specification as entry conditions for
  `MATH-UX-2`.
...
- Continue companion controlled-scope work only where it supports the L1.7A
  ...
  L1.7C-MATH/L1.7B-MAP/L1.7B-P23/SPEC-ET-1/EX-LESSON-1/GAME-UX-3A/ENGINE-OP-1/SKILLMAP-OP-1/GRAPH-UX-2 foundation, and the open
...
- Use the L1.7A readiness decision, L1.7B-C contract-and-stop outcome,
  ...
  work. Complete graph/math/reasoning integration, GATE-ENGINE-1,
...
- L1.7B-C through GATE-L1.7B closed the checkpoint product boundary only.
  ...
  SKILLMAP-OP-1 is closed as visible-route proof, and GRAPH-UX-2 is closed as
  graph/table task-shell integration proof. MATH-UX-2 through GATE-ENGINE-1
  must prove the shared task shell and remaining practice engines
  operationally before L1.7B-Q2
