...
| TASK-CONTEXT-SPEC-1 | Shared Task Context Contract | no | After `EXAM-SOURCE-AUTH-1`, define the task-shell context schema and authoring/interchange model: markdown/source-map authoring plus validated runtime `contextBlocks` and task `contextRefs`. Include fixtures and checker rejection for missing alt text, missing refs, unreferenced source blocks, answer-hint leakage, raw copied images where reconstruction is required, inconsistent captions, and internal-code exposure. |
