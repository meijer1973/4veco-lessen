...
| L-CP6A | Book 1 Chapter 1.3 v5 Alignment Remediation | no | **OPEN LESSON-TEAM TICKET.** See `lesson-ticket-L-CP6A-book1-chapter13-v5-alignment.md`. Resolve the still-open `1.3.2`/`1.3.3` source/lesson mismatch from platform CP.6a by planning and running an authorized lesson-side remediation/regeneration sprint. Chapter folders, chapter plan, chapter markdown, book aggregates, navigation, reviews, and quality refs must consistently align active-v5 `1.3.2 Marktevenwicht` and `1.3.3 Verschuivingen en nieuw evenwicht`; costs/revenue material must be preserved for Book 2. No hand-patched generated output, protected reference mutation, CP-6 closure, or Year-1 closure. |
