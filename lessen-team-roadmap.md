...
-> MATH-REFINE-1 / REASON-REFINE-1 / CHECK-Q2-PLAN (remaining planning/preparation only)
-> L1.7B-Q2
-> GATE-L1.7B-Q2
-> Scale Gate 1
```
...
- Book 2 Part A should wait until the remaining foundation blockers clear.
  ...
  GATE-ENGINE-1, and GRAPH-REFINE-1.
  Still-open blockers are L1.7B-Q2,
...
- Treat `GATE-MTU-H4` through `MTU-H4C` as closed platform answer-form
  ...
  operational integration review, and `GRAPH-REFINE-1` has closed graph
  operation-chain planning with a target-chain repair flag. The remaining
  active operational dependencies are `MATH-REFINE-1`, `REASON-REFINE-1`,
  `CHECK-Q2-PLAN`, and later `L1.7B-Q2`/`GATE-L1.7B-Q2`.
...
- Use the L2.0 exit-ticket checklist, GATE-L1.7B carried flags,
  ...
  GRAPH-REFINE-1 graph operation-chain findings, and the SYNC-4 shared
