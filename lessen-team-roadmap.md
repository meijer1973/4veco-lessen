...
-> L1.7B-Q2 (active planning selects 1.1.2; D31/copy/proof blockers remain)
-> GATE-L1.7B-Q2
-> Scale Gate 1
```
...
| L1.7B-Q2 | Exit Ticket Target-Equivalent Implementation | no | **ACTIVE PLANNING/PREFLIGHT ONLY / REQUIRED BEFORE TARGET-EQUIVALENT COMPLETION CLAIMS.** The implementation plan selects `1.1.2 Percentages en indexcijfers` as the first target-equivalent exit-ticket candidate. It must check the same target-exercise operation chain at the same cognitive level: percentage change, price-index calculation, index-to-index percentage change, and D31 index points versus percent. Planning review accepts the selection with flags: D31 must be checked as student action, generic self-check is not proof without reviewed criteria, current `Korte check`/landing copy must be repaired or scoped before target-equivalent review, and completion language remains behind `GATE-L1.7B-Q2`. This planning state authorizes no `source-data/book-1/exit-ticket/1.1.2.json` creation, generated Book 1 output, engine implementation, target-equivalent completion language, diagnostics, adaptive routing, mastery/sequencing, summative use, PV, Scale Gate 1, or product use. Requires later metadata alignment, answer-model alignment, shared task-shell support, teacher-learning-quality review, student-experience review, screenshot/interaction QA, lead review, and no generated-output hand patching. |
