...
| L-PV3 | Proof 002 - Fresh PV-Validated Surface | **2026-05-14** | **CLOSED PASS WITH HUMAN-REVIEW BOUNDARY.** `PVG4-proof-002` prepared from a bounded lesson-owned A61 table-trace pilot surface. It validates `select_table_values_trace` step order and visual anchors without claiming student-facing PV publication. HCS must decide whether this bounded pilot is sufficient diversity for proof 002. |
| L-PV4 | PV-G4 Intake Closure Packet | **2026-05-14** | **HCS PACKAGE PREPARED / HUMAN REVIEW REQUIRED.** Two lesson-owned proof records now exist under `pv-g4-proof-records/`; platform proof intake now records `2/2` and the checker passes. HCS review packet is prepared. This does not close PV-G4 and does not authorize PV machine promotion, student-facing PV projection, diagnostics, adaptive use, AI, sequencing, mastery, or summative use. |
| L-PV5 | Roadmap Sync After PV-G4 | no | **WAITING ON HCS DECISION.** After HCS records the PV-G4 decision, sync lesson/reference roadmaps, unblock or resequence L1.5B/L1.5G/L1.6/L1.7 based on the gate outcome, and record any PASS WITH CONDITIONS remediation before PV-dependent scaling resumes. |
...
| L1.5B | Layout Round 2 - Generator Items | no | Resequenced after the L-PV proof track unless explicitly scoped as non-PV/non-procedure layout work. Acts on what L1.4 surfaced + the deferred round-2 item #3 (split valkuilen pitfalls) + the deferred item #1 (per-section card accents, reclassified as generator-touching by L1.5A planner). |
| L1.5G | Three-Aspect Game Coverage | no | After L1.5B and PV-G4 closure/sync. Bring the user's graphical-game prototype into the platform; one working prototype per learning aspect (reasoning, calculation, graphical). Architectural constraint: each game accepts adaptive input via localStorage; existing 5 games get a light refactor for the same seam. Graphical-game semantics must align with PV records where they exist and with the L-PV contract/proof machinery. |
| L1.6 | Second Pipeline Regression Paragraph | no | After L1.5G. Third Book 1 paragraph build confirms Round 2 layout + the new graphical game + the adaptive-input seam survive a fresh end-to-end build, using the L-PV contract/proof machinery for at least one procedure/visual sequence where PV data exists. |
| L1.7 | Post-Layout Scaling Decision | no | After L1.6. Decide whether to expand broadly once L1.3A-C through L1.5G have proven the layout, web-doc, game, and PV proof pipelines survive independent paragraph builds; include PV readiness in the scaling decision. |
...
PV-G4 is not closed just because the two known procedure-drift cases were cleaned up. The lesson team executed L-PV0 through L-PV4 on 2026-05-14 and prepared two lesson-owned proof records. Current platform evidence now records `recorded_proof_count: 2/2` in `reports/json/procedure-visual-lesson-regression-proof-intake.json`, but HCS still has to review the packet and make the gate decision. L-PV5 remains the roadmap-sync sprint after that decision.
...
L1.5G graphical-game work must align with the Procedure-Visual Registry where PV records exist. The graphical prototype may not hard-code a separate semantic model for operations, visual states, or procedure sequences that the PV layer already defines. L1.5G now waits for PV-G4 proof closure/sync unless it is explicitly scoped as non-PV work.
...
Position: after L1.5B and PV-G4 closure/sync.
...
can replace them. L1.5G must consume the L-PV contract/proof machinery after
PV-G4, and must not assume PV machine promotion or student-facing PV projection.
...
Position: after L1.5G, using the L-PV proof machinery established before PV-G4
closure.
