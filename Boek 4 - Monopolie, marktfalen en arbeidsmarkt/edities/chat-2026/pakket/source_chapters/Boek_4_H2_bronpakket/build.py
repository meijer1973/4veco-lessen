"""Rebuild locally: python author_chapter.py (only to regenerate original manuscript),
python make_assets.py, python build.py. Edit page Markdown to revise text directly.
...
def build(pages,stem,title,kind='student'):
    ...
    # Human-readable paragraph bookmarks and explicit intra-PDF destinations.
    # This edits navigation only, never page content.
    import fitz
    with fitz.open(OUT/(stem+'.pdf')) as pdf:
        toc=pdf.get_toc()
        if kind=='student':
            names={3:'4.2.1 Welvaartseffecten van monopolie',11:'4.2.2 Prijsdiscriminatie',
                   19:'4.2.3 Marktvormen vergelijken',25:'4.2.4 Negatieve externe effecten',
                   36:'4.2.5 Positieve externe effecten',47:'4.2.6 Overheidsingrijpen bij marktfalen',
                   53:'4.2.7 Gemengde opgaven',59:'Hoofdstukoverzicht',60:'Begrippenlijst'}
            toc=[[1,'Inhoud · Marktvormen en marktfalen',1]]+[
                [level,names.get(page,label) if level==1 else label,page]
                for level,label,page in toc]
        pdf.set_toc(toc)
        for pg in pdf:
            for link in pg.get_links():
                if link.get('nameddest') and link.get('page',-1)>=0:
                    pg.update_link({'xref':link['xref'],'kind':fitz.LINK_GOTO,
                                    'from':link['from'],'page':link['page'],
                                    'to':fitz.Point(0,0),'zoom':0})
        temp=OUT/(stem+'.nav.pdf')
    ...
    print(stem,'physical:',len(doc.pages),'source sections:',len(pages),('page drift: '+str(overflow[:10])) if kind=='student' else '(flow layout)')
    if kind=='student' and (overflow or len(doc.pages)>60):raise ValueError('Student page plan or 60-page cap failed')
    return doc
if __name__=='__main__':
    if not (ROOT/'chapter-order.json').exists():
        raise FileNotFoundError('chapter-order.json is required; manuscript sources are not regenerated automatically.')
    pages=[p for f in json.loads((ROOT/'chapter-order.json').read_text()) for p in page_chunks(ROOT/f)]
    (ROOT/'4.2 Marktvormen en marktfalen – hoofdstuk.md').write_text('\n\n'.join((ROOT/f).read_text() for f in json.loads((ROOT/'chapter-order.json').read_text())),encoding='utf-8')
    build(pages,'Boek_4_H2_Marktvormen_en_marktfalen','Boek 4 · Hoofdstuk 4.2 · Marktvormen en marktfalen')
    for source,stem,title,kind in [('Antwoorden.md','Boek_4_H2_Antwoorden','Antwoorden · Marktvormen en marktfalen','answer'),('Docenteninformatie.md','Boek_4_H2_Docenteninformatie','Docenteninformatie · Marktvormen en marktfalen','teacher')]:
